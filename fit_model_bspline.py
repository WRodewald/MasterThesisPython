...
# zpk parameter
zpk_b_spline_size = 32
x_zpk = ParameterLayer.ParameterLayer(zpk_b_spline_size, 40)(inputs)
...
x_Rd   = ParameterLayer.ParameterLayer(num_samples, 1)(inputs)
Rd_var   = TemporalVarianceLayer.TemporalVarianceLayer(num_samples = num_samples, weight=10E8) 
x_Rd    = Rd_var(x_Rd)

x_gain = ParameterLayer.ParameterLayer(num_samples, 1)(inputs)
...
mse_weights = np.reshape(np.arange(1, num_overtones+1), [1,num_overtones])
mse_weights = np.power(0.8, mse_weights)
mse_weights = mse_weights / np.average(mse_weights) 
