...
from model import ZPKToMagLayer, NormalizeLayer, LFRdLayer, util, VarianceLayer
...
overtones = overtones / np.max(np.abs(overtones))
overtones = util.RelativePhaseLayer()(tf.cast(overtones, dtype='complex64'))
...
x_gain = util.DBToMagLayer()(x_gain)
x_gain = tf.keras.layers.Lambda(lambda x: tf.tile(tf.complex(x, 0.), [1, num_overtones]))(x_gain)


# Vocal tract branch
x_zpk, _, _, k = ZPKToMagLayer.ZPKToMagLayer(sample_rate, num_overtones, name='zpk')([x_zpk,inputs])

Rd_gain = util.ExponentialGainLayer()
x_Rd = Rd_gain(x_Rd)

# full model S = H_GF * H_VT * G 
x = tf.keras.layers.Multiply()([x_Rd, x_gain, x_zpk])
x = util.RelativePhaseLayer()(x)
...
fc = 1000.
lpf  = tf.cast(tf.abs(1. / (1. + 1j * fk/fc)), dtype='float32')
weights_phase = lpf * lpf

# weight 
phase_weight = 24

model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=10E-4),
    loss= lambda y_true, y_pred: util.weighted_mse_loss_phase(y_true, y_pred, weights, weights_phase, phase_weight),
    metrics=[])
...
sio.savemat('out.mat', {
    ...
    'g':g_pred,
    'Rd':Rd_pred,
    'RdOut':util.mag2db(tf.abs(RdOut_pred)).numpy(),
    'z0':z0_pred, 
    'p0': p0_pred, 
    'k':k_pred,
    'pred':predictor,
    #'var':zpk_variance.variance.numpy(),
    'resp':util.mag2db(tf.abs(response)).numpy()})
