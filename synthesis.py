...
g_pred, Rd_pred, p0_pred, z0_pred = model.predict(f0, batch_size=40000)
...
if True:
    f0_interpolator = interpolate.interp1d(t_frame, np.squeeze(f0), axis=0)
    g_interpolator  = interpolate.interp1d(t_frame, np.squeeze(g_pred),  axis=0)
    Rd_interpolator = interpolate.interp1d(t_frame, np.squeeze(Rd_pred), axis=0)
    p0_interpolator = interpolate.interp1d(t_frame, np.squeeze(p0_pred), axis=0)
    z0_interpolator = interpolate.interp1d(t_frame, np.squeeze(z0_pred), axis=0)
...
soundfile.write('out.wav', 0.707 * audio / np.max(np.abs(audio)), fs)
